...
export const useUpdateUserProfileMutation = () => {
  return useMutation({
    mutationFn: (params: { id: string; data: UserUpdateRequest }) =>
      updateUserProfile(params.id, params.data),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: userKeys.all() });
      void queryClient.invalidateQueries({ queryKey: userKeys.userById(variables.id) });
    },
  });
};
