...
import { getArtistById, createArtist, updateArtist } from '@/api/artist.api';
...
export const useCreateArtistMutation = () => {
  return useMutation({
    ...
    onSuccess: variables => {
      void queryClient.invalidateQueries({ queryKey: artistKeys.detail(variables.id) });
    },
  });
};
...
export const useUpdateArtistMutation = () => {
  return useMutation({
    mutationFn: (params: { id: string; data: ArtistUpdateRequest }) =>
      updateArtist(params.id, params.data),
