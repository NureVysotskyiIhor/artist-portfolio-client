...
export const commissionRequestKeys = {
  ...
  nearby: (params: CommissionRequestNearbyParams) =>
    ['commissionRequests', 'nearby', params] as const,
};
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({
        queryKey: commissionRequestKeys.byUser(variables.userId),
      });
    },
  });
};
...
export const useUpdateCommissionRequestMutation = () => {
  return useMutation({
    mutationFn: (params: { id: string; userId: string; data: CommissionRequestUpdateRequest }) =>
      updateCommissionRequest(params.id, params.data),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: commissionRequestKeys.detail(variables.id) });
      void queryClient.invalidateQueries({
        queryKey: commissionRequestKeys.byUser(variables.userId),
      });
    },
...
export const useUpdateCommissionRequestArtistNoteMutation = () => {
  return useMutation({
    ...
    }) => updateCommissionRequestArtistNote(params.id, params.data),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: commissionRequestKeys.all() });
      void queryClient.invalidateQueries({ queryKey: commissionRequestKeys.detail(variables.id) });
    },
  });
};
