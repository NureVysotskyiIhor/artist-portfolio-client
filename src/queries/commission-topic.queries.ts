...
import {
  getCommissionTopics,
  getCommissionTopicById,
  createCommissionTopic,
  updateCommissionTopic,
  deleteCommissionTopic,
} from '@/api/commission-topic.api';
...
export const useCreateCommissionTopicMutation = () => {
  return useMutation({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: commissionTopicKeys.all() });
    },
  });
};
...
export const useUpdateCommissionTopicMutation = () => {
  return useMutation({
    mutationFn: (params: { id: string; data: CommissionTopicUpdateRequest }) =>
      updateCommissionTopic(params.id, params.data),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({ queryKey: commissionTopicKeys.all() });
      void queryClient.invalidateQueries({ queryKey: commissionTopicKeys.detail(variables.id) });
    },
  });
};
