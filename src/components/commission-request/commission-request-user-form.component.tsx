...
import { useCreateCommissionRequestMutation, useUpdateCommissionRequestMutation } from '@/queries/commission-request.queries'
import { useUserProfileQuery } from '@/queries/users.queries'
import { commissionRequestCreateSchema, type CommissionRequestCreateFormInput } from '@/utils/validations-commission-request/commission-request-create.utils'
import { commissionRequestUserUpdateSchema, type CommissionRequestUserUpdateFormInput } from '@/utils/validations-commission-request/commission-request-user-update.utils'
...
export const CommissionRequestUserForm = ({ request, onClose }: CommissionRequestUserFormProps) => {
    ...
    createRequest(
      {
        ...
        budgetMin: data.budgetMin as number,
        budgetMax: data.budgetMax as number,
      ...
      },
      {
        ...
        onError: (error) => { if (error instanceof ApiError) toast.error('Failed to create request') },
      }
    )
  }
      ...
      >
        ...
        <div className="flex gap-3">
          <div className="flex flex-col gap-2 flex-1">
            <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Budget min *</Label>
            <Input {...updateForm.register('budgetMin', { valueAsNumber: true })} type="number" placeholder="0" />
          </div>
          <div className="flex flex-col gap-2 flex-1">
            <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Budget max *</Label>
            <Input {...updateForm.register('budgetMax', { valueAsNumber: true })} type="number" placeholder="0" />
          </div>
        </div>

        <div className="flex flex-col gap-2">
          <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Deadline *</Label>
          ...
          <div className="flex flex-col gap-3">
            {(['telegram', 'instagram', 'whatsapp'] as const).map(contact => (
              <div key={contact} className="flex flex-col gap-1">
                ...
                <Input {...updateForm.register(`contacts.${contact}`)} placeholder={`@${contact}`} />
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2">
        <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Topic *</Label>
        <select
      ...
      <div className="flex flex-col gap-2">
        <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Title *</Label>
        <Input
      ...
      <div className="flex flex-col gap-2">
        <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Location</Label>
      ...
      <div className="flex flex-col gap-2">
        <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Description *</Label>
        <textarea
      ...
      <div className="flex gap-3">
        <div className="flex flex-col gap-2 flex-1">
          <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Budget min *</Label>
          <Input {...createForm.register('budgetMin', { valueAsNumber: true })} type="number" placeholder="0" />
        </div>
        <div className="flex flex-col gap-2 flex-1">
          <Label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Budget max *</Label>
          <Input {...createForm.register('budgetMax', { valueAsNumber: true })} type="number" placeholder="0" />
        </div>
      </div>
